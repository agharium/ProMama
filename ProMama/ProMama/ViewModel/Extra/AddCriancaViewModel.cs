...
namespace ProMama.ViewModel.Extra
{
    class AddCriancaViewModel : ViewModelBase
    {
        ...
        private async void AddCrianca()
        {
            ...
            {
                if (await this._messageService.ConfirmationDialog("Você tem certeza que esta é a data de nascimento da criança? Você não poderá alterar esta informação posteriormente.")){
                    ...
                    {
                        await this._messageService.AlertDialog("Ocorreu um erro.");
                    }
                }
            }
        }
